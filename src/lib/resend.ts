...
export async function addContactToResend(email: string, source: string = 'landing_page') {
  ...
  if (!resend || !apiKey) {
    console.log('[Resend] RESEND_API_KEY is not set in environment.');
    return { success: false, message: 'RESEND_API_KEY is not configured in environment variables.' };
  }
  ...
  let contactErrorMsg: string | null = null;
  ...
  try {
    ...
    if (contactError) {
      contactErrorMsg = contactError.message;
      console.warn('[Resend Contacts Error]:', contactError.message);
    } else {
      contactSuccess = true;
      console.log('[Resend Contact Created]:', contactData?.id);
    }
  } catch (contactEx) {
    contactErrorMsg = contactEx instanceof Error ? contactEx.message : 'Unknown contact error';
    console.warn('[Resend Contacts Exception]:', contactErrorMsg);
  }

  // 2. Send OnRead VIP Confirmation Email
  const configuredFrom = process.env.RESEND_FROM_EMAIL;
  // If configuredFrom is provided, use it. Otherwise use the default testing sender.
  const fromEmail = configuredFrom || 'OnRead <onboarding@resend.dev>';
  ...
  try {
    console.log(`[Resend] Attempting to send confirmation email from: "${fromEmail}" to: "${normalizedEmail}"`);
    ...
    if (emailError) {
      ...
      // If custom sender failed (e.g. unverified domain) and we didn't use onboarding@resend.dev, try fallback
      if (fromEmail !== 'OnRead <onboarding@resend.dev>') {
        console.log('[Resend] Retrying email with default test sender "OnRead <onboarding@resend.dev>"...');
        const retryResult = await resend.emails.send({
          from: 'OnRead <onboarding@resend.dev>',
          to: normalizedEmail,
          subject: "💀 You're in Drop #01 — OnRead Aura Judge",
          html: htmlContent,
        });
        ...
        if (retryResult.data?.id) {
          ...
          emailErrorMsg = null;
          console.log('[Resend Email Sent via Fallback Sender]. ID:', retryResult.data.id);
        } else if (retryResult.error) {
          console.error('[Resend Retry Error]:', retryResult.error.message);
        }
      }
    ...
    emailErrorMsg = emailEx instanceof Error ? emailEx.message : 'Exception sending email';
